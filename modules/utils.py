...
def write_model_list(model_list, outputfile, dataset_name):
    for (ii, model) in enumerate(model_list):
        ...
        model_obj= model[0]
        write_data_to_h5(outputfile, model_obj.direction_vector, dataset_name + "%04d/direction_vector" %ii, overwrite=True)
        write_data_to_h5(outputfile, model_obj.support_vector, dataset_name + "%04d/support_vector" %ii, overwrite=True)
        error = model[1]
...
def from_lists_to_precision_recall(gt_list, test_list):
    ...
    if float(len(true_positives)+len(false_positives)) == 0:
        ...
        fscore = 2*len(true_positives)/float((2*len(true_positives)+len(false_negatives)+len(false_positives)))
    error_dic = {}
